...
const URL = 'https://sdarot.today/ajax/watch';

let jar = request.jar();

let headers = {
  'User-Agent': UA,
  Origin: 'https://sdarot.today',
  Referer: 'https://sdarot.today/watch/',
};

async function watch(options) {
  console.log(
    `starting sid\\season\\episode: ${options.SID}\\${options.season}\\${options.episode}`,
  );
      ...
      .on('response', (response) => {
        ...
        let interval = setInterval(() => {
          process.stdout.write(
            '\rdownload progress: ' + Math.round(total / contentLength * 100) + '%',
          );
        }, 1000);
